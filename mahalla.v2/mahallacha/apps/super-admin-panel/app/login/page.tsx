...
export default function SuperAdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!otp.trim()) {
        const otpResponse = await requestLoginOtp("super-admin", cleanPhone);
      ...
      const verifyResponse = await verifyLoginOtp("super-admin", cleanPhone, otp);
      ...
      if (!panelAllowsRole("super-admin", tokens.user.role)) {
        throw new ApiError("Bu akkaunt Super Admin paneliga kira olmaydi.", 403, null);
      }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-sm space-y-4">
        <h1 className="text-xl font-semibold text-center">Super Admin - Kirish</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="123456"
              className="w-full rounded border border-slate-200 bg-white px-3 py-2 text-sm tracking-widest text-slate-900"
            />
          </div>
          ...
          <p className="text-[11px] text-gray-500 text-center">
            Privileged amallar uchun 2FA talab qilinadi.
          </p>
        </form>
      </div>
    </main>
