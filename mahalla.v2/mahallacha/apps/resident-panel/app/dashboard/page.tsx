...
export default function ResidentDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (!user || !panelAllowsRole("resident", user.role)) {
      clearSession();
    ...
    const load = async () => {
      ...
      try {
        const [postRes, complaintRes, reportRes] = await Promise.all([
          listPosts({ status: "PUBLISHED", page: 1, pageSize: 5 }),
  ...
  const ratedReportsCount = useMemo(
    () => reports.reduce((acc, report) => acc + (report.ratings?.length ?? 0), 0),
    [reports]
  );

  return (
    <main className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold">Mening mahallam</h1>
      {error ? <p className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p> : null}

      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-lg border bg-white p-4">
          <p className="text-sm text-gray-500">Yangi e'lonlar</p>
          <p className="mt-2 text-2xl font-semibold">{loading ? "..." : posts.length}</p>
        </div>
        <div className="rounded-lg border bg-white p-4">
          <p className="text-sm text-gray-500">Mening shikoyatlarim</p>
          <p className="mt-2 text-2xl font-semibold">{loading ? "..." : complaints.length}</p>
        </div>
        <div className="rounded-lg border bg-white p-4">
          <p className="text-sm text-gray-500">Baholangan hisobotlar</p>
          <p className="mt-2 text-2xl font-semibold">{loading ? "..." : ratedReportsCount}</p>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="rounded-lg border bg-white p-4">
          <h2 className="text-lg font-semibold mb-3">Mening shikoyatlarim</h2>
          <ul className="space-y-2 text-sm">
            {complaints.map((c) => (
