...
export default function HokimiyatLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!panelAllowsRole("hokimiyat", tokens.user.role)) {
        throw new ApiError("Bu akkaunt Hokimiyat paneliga kira olmaydi.", 403, null);
      }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="w-full max-w-md rounded-lg border bg-white p-6 shadow-sm space-y-4">
        <h1 className="text-xl font-semibold text-center">Hokimiyat - Kirish</h1>
